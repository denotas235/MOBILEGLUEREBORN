...
/**
 * Implementacao pura de Simplex Noise em Java.
 * Nao usa bibliotecas externas. Thread-safe (sem estado mutavel compartilhado).
 * Baseada na implementacao de Stefan Gustavson (dominio publico).
 */
public final class SimplexNoise {

    private static final int[] PERM;
    private static final int[] PERM12;
    ...
    public static double noise(double xin, double yin) {
        ...
        double n0,n1,n2;
        double t0=0.5-x0*x0-y0*y0; n0= t0<0 ? 0 : (t0*=t0, t0*t0*dot(GRAD3[PERM12[ii+PERM[jj]]],x0,y0));
        double t1=0.5-x1*x1-y1*y1; n1= t1<0 ? 0 : (t1*=t1, t1*t1*dot(GRAD3[PERM12[ii+i1+PERM[jj+j1]]],x1,y1));
        double t2=0.5-x2*x2-y2*y2; n2= t2<0 ? 0 : (t2*=t2, t2*t2*dot(GRAD3[PERM12[ii+1+PERM[jj+1]]],x2,y2));
        return 70.0*(n0+n1+n2);
    }

    public static double noise(double xin, double yin, double zin) {
        double F3=1.0/3.0, G3=1.0/6.0;
        double s=(xin+yin+zin)*F3;
        int i=(int)Math.floor(xin+s),j=(int)Math.floor(yin+s),k=(int)Math.floor(zin+s);
        double t=(i+j+k)*G3, X0=i-t, Y0=j-t, Z0=k-t;
        ...
        double n0,n1,n2,n3;
        double t0=0.6-x0*x0-y0*y0-z0*z0; n0=t0<0?0:(t0*=t0,t0*t0*dot(GRAD3[PERM12[ii+PERM[jj+PERM[kk]]]],x0,y0,z0));
        double t1=0.6-x1*x1-y1*y1-z1*z1; n1=t1<0?0:(t1*=t1,t1*t1*dot(GRAD3[PERM12[ii+i1+PERM[jj+j1+PERM[kk+k1]]]],x1,y1,z1));
        double t2=0.6-x2*x2-y2*y2-z2*z2; n2=t2<0?0:(t2*=t2,t2*t2*dot(GRAD3[PERM12[ii+i2+PERM[jj+j2+PERM[kk+k2]]]],x2,y2,z2));
        double t3=0.6-x3*x3-y3*y3-z3*z3; n3=t3<0?0:(t3*=t3,t3*t3*dot(GRAD3[PERM12[ii+1+PERM[jj+1+PERM[kk+1]]]],x3,y3,z3));
