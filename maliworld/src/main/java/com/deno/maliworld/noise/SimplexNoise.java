...
/**
 * Simplex Noise puro em Java — sem bibliotecas externas.
 * Thread-safe (sem estado mutavel partilhado).
 */
public final class SimplexNoise {
    ...
    private static double d2(int[] g, double x, double y) { return g[0]*x + g[1]*y; }
    private static double d3(int[] g, double x, double y, double z) { return g[0]*x + g[1]*y + g[2]*z; }
    ...
    public static double noise(double xin, double yin) {
        ...
        if (t0<0) { n0=0.0; } else { t0*=t0; n0=t0*t0*d2(GRAD3[PERM12[ii+PERM[jj]]],x0,y0); }
        double t1=0.5-x1*x1-y1*y1;
        if (t1<0) { n1=0.0; } else { t1*=t1; n1=t1*t1*d2(GRAD3[PERM12[ii+i1+PERM[jj+j1]]],x1,y1); }
        double t2=0.5-x2*x2-y2*y2;
        if (t2<0) { n2=0.0; } else { t2*=t2; n2=t2*t2*d2(GRAD3[PERM12[ii+1+PERM[jj+1]]],x2,y2); }
        return 70.0*(n0+n1+n2);
    }

    public static double noise(double xin, double yin, double zin) {
        final double F3=1.0/3.0, G3=1.0/6.0;
        ...
        if (t0<0) {n0=0.0;} else {t0*=t0; n0=t0*t0*d3(GRAD3[PERM12[ii+PERM[jj+PERM[kk]]]],x0,y0,z0);}
        double t1=0.6-x1*x1-y1*y1-z1*z1;
        if (t1<0) {n1=0.0;} else {t1*=t1; n1=t1*t1*d3(GRAD3[PERM12[ii+i1+PERM[jj+j1+PERM[kk+k1]]]],x1,y1,z1);}
        double t2=0.6-x2*x2-y2*y2-z2*z2;
        if (t2<0) {n2=0.0;} else {t2*=t2; n2=t2*t2*d3(GRAD3[PERM12[ii+i2+PERM[jj+j2+PERM[kk+k2]]]],x2,y2,z2);}
        double t3=0.6-x3*x3-y3*y3-z3*z3;
        if (t3<0) {n3=0.0;} else {t3*=t3; n3=t3*t3*d3(GRAD3[PERM12[ii+1+PERM[jj+1+PERM[kk+1]]]],x3,y3,z3);}
